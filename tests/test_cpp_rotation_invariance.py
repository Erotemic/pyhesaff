...
def TEST_figure2(imgBGR, kpts, desc, sel, fnum=2):
    ...
    show_keypoints(
        imgBGR, kpts, sifts=None, sel_fx=sel, ori=False, **viz_kwargs
    )
    _annotate_kpts(kpts, sel, ori=True, **viz_kwargs)
    draw_feat_row(
        imgBGR, sel, kpts[sel], sift, fnum=fnum, nRows=2, nCols=3, px=3
    )
...
def TEST_keypoint(imgBGR, img_fpath, kpts, desc, sel, fnum=1, figtitle=''):
    ...
    # ----------------------#
    f1_loc = TEST_figure1(
        wpatch, gradx, grady, gmag, gori, hist, centers, fnum=fnum
    )
...
def test_cpp_rotinvar_main():
    ...
    [kpts1], [desc1] = pyhesaff.detect_feats_list(
        [img_fpath], rotation_invariance=False
    )
    [kpts2], [desc2] = pyhesaff.detect_feats_list(
        [img_fpath], rotation_invariance=True
    )
    np.set_printoptions(
        threshold=5000, linewidth=5000, precision=8, suppress=True
    )
    ...
    print(
        '\n----\n'.join(
            [
                str(k1) + '\n' + str(k2)
                for k1, k2 in zip(kpts1[0:10], kpts2[0:10])
            ]
        )
    )
