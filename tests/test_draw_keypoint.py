...
def test_draw_keypoint_main():
    ...
    def test_shape(ori=0, skew=0, xscale=1, yscale=1, pnum=(1, 1, 1), fnum=1):
        ...
        kpts, sifts = test_keypoint(
            ori=ori, skew=skew, xscale=xscale, yscale=yscale
        )
        ax = df2.gca()
        square_axis(ax)
    ...
    MAX_ORI = float(ub.argval('--max-ori', DOWN + TAU - 0.2))
    MIN_X = 0.5
    MAX_X = 2
    ...
    for row, col in it.product(range(nRows), range(nCols)):
        ...
        kpts, sifts = test_shape(
            pnum=next(pnum_), ori=ori, skew=skew, xscale=xsca, yscale=ysca
        )
        # print('+----')
